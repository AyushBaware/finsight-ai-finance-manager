...
import { ArrowDownRight, ArrowUpRight, PieChart, Plus, TrendingUp } from "lucide-react"
    ...
    <div className="space-y-6">
      <div className="theme-hero rounded-2xl p-6 shadow-lg">
        ...
        <p className="mt-2 opacity-90">
          You have Rs {leftoverMoney.toLocaleString()} leftover this month. Let AI help you make smart decisions.
        </p>
      </div>
        ...
        <Card padding="lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">Rs {financialData.monthlyIncome.toLocaleString()}</h3>
            </div>
            <div className="rounded-full bg-green-100 p-3 text-green-600 dark:bg-green-900/30 dark:text-green-400"><ArrowUpRight size={24} /></div>
          </div>
        </Card>
        <Card padding="lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">Rs {financialData.totalExpenses.toLocaleString()}</h3>
            </div>
            <div className="rounded-full bg-red-100 p-3 text-red-600 dark:bg-red-900/30 dark:text-red-400"><ArrowDownRight size={24} /></div>
          </div>
        </Card>
        <Card padding="lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">Rs {leftoverMoney.toLocaleString()}</h3>
            </div>
            <div className="rounded-full bg-indigo-100 p-3 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400"><TrendingUp size={24} /></div>
          </div>
        </Card>
        <Card padding="lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{savingsRate}%</h3>
            </div>
            <div className="rounded-full bg-purple-100 p-3 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400"><PieChart size={24} /></div>
          </div>
        </Card>
      </section>
          ...
          {recentExpenses.length > 0 ? recentExpenses.map((expense) => (
            <Card key={expense.id} padding="sm" className="transition-shadow hover:shadow-md">
              <div className="flex items-center justify-between">
                ...
                <p className="font-semibold text-red-500 dark:text-red-400">- Rs {Number(expense.amount || 0).toLocaleString()}</p>
              </div>
            </Card>
          )) : (
            <Card padding="sm" className="text-center text-gray-500"><p>No expenses yet. Add one to get started!</p></Card>
          )}
        </div>
      </section>
    </div>
